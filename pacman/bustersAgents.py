    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        mostLikely = [dist.argMax() for dist in livingGhostPositionDistributions]
        minAction, minDist = legal[0],float("inf")
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            for ghost in mostLikely:
                ghostd = self.distancer.getDistance(successor, ghost)
                if ghostd < minDist:
                    minAction, minDist = action, ghostd
        return minAction
